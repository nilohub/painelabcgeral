...
CREATE OR REPLACE FUNCTION public.get_declining_products(
  ...
  CASE WHEN l.second_min IS NULL THEN '-' 
       ELSE (l.second_min / 100)::text || '/' || lpad((l.second_min % 100)::text, 2, '0')
            || CASE WHEN l.second_min <> l.second_max 
                    ...
                    ELSE '' END
  END AS second_period_label,
  a.first_sales,
  a.second_sales,
  a.first_profit,
  a.second_profit,
  CASE WHEN a.first_sales > 0 THEN ROUND(((a.second_sales - a.first_sales) / a.first_sales * 100)::numeric, 2) ELSE NULL END AS sales_delta_pct,
  CASE WHEN a.first_profit > 0 THEN ROUND(((a.second_profit - a.first_profit) / a.first_profit * 100)::numeric, 2) ELSE NULL END AS profit_delta_pct
FROM agg a
CROSS JOIN labels l
...
ORDER BY (a.second_sales - a.first_sales) ASC;
$$;

GRANT EXECUTE ON FUNCTION public.get_declining_products(integer, integer, text[], text) TO anon, authenticated, service_role;
