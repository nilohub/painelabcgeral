...
export function SalesCharts({ data }: SalesChartsProps) {
  const monthlyData = useMemo(() => {
    const grouped: Record<number, { sales: number; profit: number; quantity: number }> = {};

    data.forEach((item) => {
      if (!grouped[item.month]) {
        grouped[item.month] = { sales: 0, profit: 0, quantity: 0 };
      }
  ...
  const storeData = useMemo(() => {
    const grouped: Record<string, { sales: number; profit: number; quantity: number }> = {};

    data.forEach((item) => {
      if (!grouped[item.store]) {
        grouped[item.store] = { sales: 0, profit: 0, quantity: 0 };
      }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card className="border-border bg-card shadow-card lg:col-span-2">
        <CardHeader>
          <CardTitle className="text-lg">Vendas e Lucro por Mês</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-border bg-card shadow-card lg:col-span-2">
        <CardHeader>
          <CardTitle className="text-lg">Comparativo por Loja</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
